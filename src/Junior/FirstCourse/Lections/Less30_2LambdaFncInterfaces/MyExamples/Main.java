...
import static Junior.FirstCourse.Lections.Less30_2LambdaFncInterfaces.MyExamples.CarsOperations.filtrationCars;
import static Junior.FirstCourse.Lections.Less30_2LambdaFncInterfaces.MyExamples.CarsOperations.info;
...
//        }
        ...
        //Function<T,R>
        Function<Integer, String> convert = x-> String.valueOf(x) + " единиц (теперь это стринг)";
        int test1 = 777;
        String stringTest1 = convert.apply(test1);
        System.out.println(stringTest1); //"777 единиц (теперь это стринг)"

        //Consumer<T>
        Consumer<Integer> printer = x-> System.out.printf("%d единиц \n", x);
...
//        System.out.printf("Total cars : %d. \n", getCounter());
        ...
        ArrayList<Car> ourCars = createThreeCars(() -> new Car("Car","Civil",120,180));
        ourCars.forEach(car -> info(car));

        changeCar(car1,car -> car.setName("Speedy"));
        info(car1);

        System.out.println("TEST----------------------------------------------");
        Consumer<Car> carConsumer = car -> {
            car.setName("CivilCar");
            car.setUse("Civil");
            car.setHorsePower(120);
            car.setSpeed(180);
        };
        //carConsumer.accept(ourCars.get(0));
        ourCars.forEach(carConsumer); //car -> {carConsumer.accept(car);} //carConsumer::accept
        ourCars.forEach(CarsOperations::info); //car -> info(car)
    ...
    //метод который будет изменять машину
